...
import { URLs } from './user-data/urls.js';
  ...
  const { independentConsulting, personal, selfLearning, academe } =
    projects;
  ...
  function populateBlogs(items, id) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      var tagIndex = items[i].description.indexOf('<img'); // Find where the img tag starts
      var srcIndex = items[i].description.substring(tagIndex).indexOf('src=') + tagIndex; // Find where the src attribute starts
      var srcStart = srcIndex + 5; // Find where the actual image URL starts; 5 for the length of 'src="'
      var srcEnd = items[i].description.substring(srcStart).indexOf('"') + srcStart; // Find where the URL ends
      var src = items[i].description.substring(srcStart, srcEnd); // Extract just the URL
      img.src = src
  ...
  populateProjects(independentConsulting, "independentConsulting");
  populateProjects(personal, "personal");
  populateProjects(selfLearning, "selfLearning");
  populateProjects(academe, "academe");
